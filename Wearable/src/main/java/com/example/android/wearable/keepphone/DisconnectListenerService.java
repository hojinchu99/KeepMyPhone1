...
import com.example.android.wearable.findphone.R;
 ...
 * other possible cases. To this end, the phone app has registered itself to provide the "ㅏKeep_me"
...
public class DisconnectListenerService extends WearableListenerService
    ...
    private static final String TAG = "KeepPhoneApp";

    private static final int MULTIPLE_PHONE_NOTIFICATION_ID = 1;

    /* the capability that the phone app would provide */
    private static final String Keep_ME_CAPABILITY_NAME = "keep_me";
    ...
    public void onConnectedNodes(List<Node> connectedNodes) {
        ...
        // "keep_me" capability and are directly connected.
    ...
    private void setOrUpdateNotification() {
        Wearable.CapabilityApi.getCapability(
                mGoogleApiClient, Keep_ME_CAPABILITY_NAME,
                CapabilityApi.FILTER_REACHABLE).setResultCallback(
                new ResultCallback<CapabilityApi.GetCapabilityResult>() {
                    @Override
                    public void onResult(CapabilityApi.GetCapabilityResult result) {
                        if (result.getStatus().isSuccess()) {
                            updateKeepMeCapability(result.getCapability());
                        } else {
                            Log.e(TAG,
    ...
    private void updateKeepMeCapability(CapabilityInfo capabilityInfo) {
            ...
            for (Node node : connectedNodes) {
                ...
                if (node.isNearby()) {
                    ((NotificationManager) getSystemService(NOTIFICATION_SERVICE))
                            .cancel(MULTIPLE_PHONE_NOTIFICATION_ID);
                }
            }
        }
    }
    ...
    private void setupLostConnectivityNotification() {
        ...
        ((NotificationManager) getSystemService(NOTIFICATION_SERVICE))
                .notify(MULTIPLE_PHONE_NOTIFICATION_ID, card);
    }
